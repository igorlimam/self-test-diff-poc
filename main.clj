...
(defn search-diff [modelo-stream modelo-block gerado-stream gerado-block]
  (let [modelo (line-seq modelo-stream)
    ...
    (if (not= (:h modelo-block) (:h gerado-block))
      (make-diff (get-block-vec modelo modelo-block) (get-block-vec gerado gerado-block))
      [])))
...
(defn -main
  ...
  (let [modelo-tmp-file (stream->temp-file (str->reader "0001|GOUM\n0000| OYOYOLDRUSTY") (hash "modeloHash"))
    ...
    (println (set/intersection (set (keys m)) (set (keys g))))
    (println (set/difference (set (keys m)) (set (keys g))))
    (println (set/difference (set (keys g)) (set (keys m))))
    (doseq [common-key (vec (set/intersection (set (keys m)) (set (keys g))))]
      (search-diff (io/reader modelo-tmp-file) (get m common-key) (io/reader gerado-tmp-file) (get g common-key)))))
